...
#start Module:
def Start():
	forward = easygui.buttonbox('说在前面：本程序灵感来源于Bilibili-GoldenEggs作品《无尽循环》，一些图片来源于 《无尽循环》，该程序仅用于娱乐和搞笑。有Bug很正常，这里引用Mojang&鲁迅名言 （bushi）：其实世上本没有特性,BUG多了,也变成了特性。',choices=['好'],title=['Start'])
	start = easygui.buttonbox('确定开始？无尽循环，即将开始！【有图别点，不然凉拌】',choices=['开始','我怂了'],title='Q0')
	if start == '我怂了' :
...
class First():
	def Q1():
		q1_1 = easygui.buttonbox('一觉起来，你看到今天是2019年5月31日，外面雨很大，你拿上你新磨得钻石剑，准备出门。你偶然看到南瓜灯，猛然想起了一个带着南瓜面具的杀手，你没多想，直接出门了。',choices=['继续'],title='1-Q1.1')
	...
	def Q4():
		...
		q4 = easygui.buttonbox('你没多犹豫，直接跳了下去。里面是一面墙，上面有几行字。你细细看着，吓了一大跳。你想到了那个神秘人。你回头，看到了那套神秘人的装备——南瓜头套,和一套金甲。旁边的箱子装着一些武器，和一个火箭喷射器。你没有退路，穿上了它。你头一昏，醒来时已经在外面了。你飞到了你家，确实有一个和自己一摸一样的人。你换上金甲，拿出了斧头，一把砍了下去。',choices=['砍他！（%20几率）','直接失败'],image=['Image/sjzz.png'],title=['1-Q4'])
		if q4 == '砍他！（%20几率）':
			if ran <= 1:
				First.KillWin()
			elif ran > 1:
	...
	def Q7():
		ran = random.randint(0,9)
		q7 = easygui.buttonbox('他跳下去了！你十分焦急，跟着他一跳，没想到，传送门神秘的消失了，你直直坠下，掉入了一地洞。你走过一弯又一弯，看到了另一面墙。那，就是所谓的‘时间表’。你毛骨悚然，恍然大悟，你决定',choices=['当救兵（80%）','直接失败'],title=['1-Q7'],image=['Image/sjb-1.png'])
		if q7 == '当救兵（80%）':
			if ran <= 1:
				Third.heroDie1()
			elif ran >= 2:
	...
	def End():
		global end
		ran = random.randint(0,9)
		filepath = r"E:/Udisk/MyText/Endlessloop·Item 1[N]/Sound/zero.mp3";
		pygame.mixer.init()
		# 加载音乐
		...
		if end <= 1:
				...
				q9_2 = easygui.buttonbox('【             假结局              】 【BGM：洛天依-〇】（剧情杀？试试再来一次！）',choices=['重来（80%第一循环，20%第三循环）'],title=['BadEnd0'])
				time.sleep(0)
				...
				if q9_3 == '告诉，真相？':
					...
					pygame.mixer.music.play(start=0.0)
					q9_2 = easygui.buttonbox('【             假结局              】 【BGM：洛天依-〇】（剧情杀？试试再来一次！）',choices=['重来（80%第一循环，20%第三循环）'],title=['BadEnd1'])
					time.sleep(0)
					pygame.mixer.music.stop()
					end = end + 1
					if q9 == "重来（80%第一循环，20%第三循环）":
						if ran <= 2:
							Third.Q1()			
						elif ran >= 3:
							First.Q1()
					...
					pygame.mixer.music.play(start=0.0)
					q9_4 = easygui.buttonbox('窗外阵阵鸟叫声，你看表，今天是20XX年6月1日，天气晴。你再次来到了那个时间表的位置，藤蔓已然消失，你进了洞，只见那副墙上，赫然写着：第四循环体……【真结局】 【BGM：洛天依-〇】',choices=['第一循环(通关奖励：任意选）','第二循环','第三循环','彩蛋'],title=['GoodEnd'])
					if q9_4 == '第一循环':
						First.Q1()
						pygame.mixer.music.stop()
					elif q9_4 == '第二循环':
			...
			pygame.mixer.music.play(start=0.0)
			q9_6 = easygui.buttonbox('【             假结局              】 【BGM：洛天依-〇】（剧情杀？试试再来一次！）',choices=['重来（80%第一循环，20%第三循环）'],title=['BadEnd2'])
			time.sleep(0)
			pygame.mixer.music.stop()
			end = end + 1
...
class Third():
	...
	def surprise():
			surpr=easygui.buttonbox('‘大人，这是我再次策划的好戏。’你到了一个未知的地方,看见两个人在讲话。‘A，B，和C三个人在这个地图里，相杀相救，被假的诅咒和真的时间表所误导，无限循环永无止境，有1,2,3循环……’‘这保险吗？你亲爱的演员又出来了。’‘欧，大人，这个传送门可能有一点故障，我很快就修好。（你被传送回初始循环）【恭喜触发彩蛋结局】【BGM：洛天依——〇】',choices=['继续'],title=['彩蛋'])
			if surpr == '继续':
				First.Q1()
			else:
				Exit()
	def Q4():
		q4 = easygui.buttonbox('你没多犹豫，直接跳了下去。里面是一面墙，上面有几行字。你细细看着，吓了一大跳。你想到了那个神秘人。你回头，看到了那套神秘人的装备——南瓜头套,和一套金甲。旁边的箱子装着一些武器，和一个火箭喷射器。你没有退路，穿上了它。你头一昏，醒来时已经在外面了。你飞到了你家，确实有一个和自己一摸一样的人。你换上金甲，拿出了斧头，一把砍了下去。',choices=['砍他！（%20几率）','直接失败'],image=['Image/sjzz.png'],title=['3-Q4'])
		ran = random.randint(0,9)
		if q4 == '砍他！（%20几率）':
			if ran <= 1:
				First.KillWin()
			...
			Exit()
	def Q5():
		q5=easygui.buttonbox('他侥幸躲了过去。你气急败坏，紧紧跟着他。你手上已经转备好了剧毒药水，只要他一犹豫，他将被毒杀。时机到了，你拿起药水，狠狠丢了过去。',choices=['砸他！（%20几率）','直接失败'],title=['3-Q5'])
		ran = random.randint(0,9)
		if q5 == '砸他！（%20几率）':
			if ran <= 1:
				First.KillWin()
	...
	def Q6():
		...
		if q6 == '射死他！（%20几率）':
			...
			Exit()
	def Q7():
		q7 = easygui.buttonbox('他跳下去了！你十分焦急，跟着他一跳，没想到，传送门神秘的消失了，你直直坠下，掉入了一地洞。你走过一弯又一弯，看到了另一面墙。那，就是所谓的‘时间表’。你毛骨悚然，恍然大悟，你决定',choices=['当救兵（80%）','直接失败'],title=['3-Q7'],image=['Image/sjb-3.png'])
		ran = random.randint(0,9)
		if q7 == '当救兵（80%）':
			if ran <= 1:
				Third.heroDie1()
			...
			Exit()
	def Q8():
		q8 = easygui.buttonbox('你在原来的地方放下了那个告示牌，上面写‘别回头’。他还是跟上来了。你将他引到了一片平地，丢下了你珍藏的红宝石，静候他上门。',choices=['引诱（80%）','直接失败'],title=['3-Q8'])
		ran = random.randint(0,9)
		if q8 == '引诱（80%）':
			if ran <= 1:
				Third.heroDie2()
			...
			Exit()
	def End():
		filepath = r"E:/Udisk/MyText/Endlessloop·Item 1[N]/Sound/zero.mp4";
		pygame.mixer.init()
		# 加载音乐
		global end
		ran = random.randint(0,9)
		if end <= 1:
				...
				pygame.mixer.music.load(filepath)
				pygame.mixer.music.play(start=0.0)
				q9_2 = easygui.buttonbox('【             假结局              】 【BGM：洛天依-〇】（剧情杀？试试再来一次！）',choices=['重来（40%第一循环，60%第三循环）'],title=['BadEnd3'])
				end += 1
				time.sleep(0)
				pygame.mixer.music.stop()
			...
			if q9_3 == '告诉，真相？':
				easygui.buttonbox('你迅速地回答他，“我们不要在自相残杀了，我们需要......”话未说完，他拿起剑就把你砍下深崖，你还未反应过来，当场暴毙。',choices=['继续'],title=['died'])
				q9_2 = easygui.buttonbox('【             假结局              】 【BGM：洛天依-〇】（剧情杀？试试再来一次！）',choices=['重来（40%第一循环，60%第三循环）'],title=['BadEnd4'])
				time.sleep(0)
				pygame.mixer.music.stop()
				end += 1
				if q9 == "重来（40%第一循环，60%第三循环）":
					if ran <= 5:
						Third.Q1()			
					elif ran >= 6:
						first.Q1()
			...
			q9_5 = easygui.buttonbox('你和他走上了顶端，他和你一样，问了"你是谁？你到底想干什么？"。你迅速地回答他，“我们不要在自相残杀了，我们需要......”话未说完，他拿起剑就把你砍下深崖，你还未反应过来，当场暴毙。',choices=['继续'],title=['died'])
			pygame.mixer.music.play(start=0.0)
			q9_6 = easygui.buttonbox('【             假结局              】 【BGM：洛天依-〇】',choices=['重来（40%第一循环，60%第三循环）'],title=['BadEnd5'])
			time.sleep(0)
			pygame.mixer.music.stop()
			end = end + 1
